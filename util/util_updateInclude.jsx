#includepath "/d/Google Drive/Adobe Scripting/extendscript_lib/common/file";
#include "as_singleLibraryJSXINC.jsxinc";
#include "as_getFileExtension.jsxinc";
#include "as_fileWrite.jsxinc";

#includepath "/d/Google Drive/Adobe Scripting/extendscript_lib/common/loop";
#include "as_loopDirectories.jsxinc";
#include "as_loopFiles.jsxinc";

#includepath "/d/Google Drive/Adobe Scripting/extendscript_lib/common/data";
#include "as_arrayCheckValue.jsxinc";

as_singleLibraryJSXINC(Folder("/d/Google Drive/Adobe Scripting/extendscript_lib/ae"));
as_singleLibraryJSXINC(Folder("/d/Google Drive/Adobe Scripting/extendscript_lib/ai"));
as_singleLibraryJSXINC(Folder("/d/Google Drive/Adobe Scripting/extendscript_lib/ps"));
as_singleLibraryJSXINC(Folder("/d/Google Drive/Adobe Scripting/extendscript_lib/common"));
as_singleLibraryJSXINC(Folder("/d/Google Drive/Adobe Scripting/extendscript_lib"));